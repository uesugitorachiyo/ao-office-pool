# AO Office Pool v0.1.3 maintainer publication
...
`v0.1.0`, `v0.1.1`, and `v0.1.2` are unsupported and superseded by v0.1.3. Preserve their
...
$AssetName = 'ao-office-pool-v0.1.3-windows-x86_64.zip'
...
Keep audit evidence only under ignored `.local/publication-v0.1.3/`. Inventory
...
2. Create and push an annotated `v0.1.3` tag at `$SourceCommit`, not at the
   later metadata-only commit.
3. Create a non-draft, non-prerelease v0.1.3 release with exactly the archive
   and sidecar.
...
Stop on any readback drift. Never move the tag or replace published v0.1.3
asset bytes.
