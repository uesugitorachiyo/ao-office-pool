...
def _require_task_root_path_budget(task_root):
    authority = task_root / "temp" / "tmp12345678" / "pool" / "runtime" / "receipts"
    record = authority / ("a" * 64 + ".receipt.json")
    temporary = record.with_name(f".{record.name}.{'0' * 32}.tmp")
    length = len(str(temporary))
    if length >= LEGACY_WINDOWS_PATH_LIMIT:
        raise SystemExit(
            "task-root-too-long "
            f"effective-descendant-length={length} "
            f"limit={LEGACY_WINDOWS_PATH_LIMIT - 1}; "
            "choose a shorter drive-absolute --task-root"
        )


def _require_fixed_ntfs_task_root(task_root):
    root = task_root.anchor
    kernel = ctypes.windll.kernel32
    if not root or kernel.GetDriveTypeW(root) != DRIVE_FIXED:
        raise SystemExit("task-root-volume-not-fixed-ntfs")
    file_system = ctypes.create_unicode_buffer(32)
    if not kernel.GetVolumeInformationW(
        root, None, 0, None, None, None, file_system, len(file_system)
    ):
        raise ctypes.WinError(ctypes.get_last_error())
    if file_system.value.casefold() != "ntfs":
        raise SystemExit("task-root-volume-not-fixed-ntfs")
...
def _supervise(args):
    ...
    _require_fixed_ntfs_task_root(task_root)
    _require_task_root_path_budget(task_root)
