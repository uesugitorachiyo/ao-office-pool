...
from scripts import run_windows_tests as windows_test_runner
...
class WindowsTestRunnerTests(unittest.TestCase):
    ...
    def test_rejects_task_root_without_safe_descendant_path_budget(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary) / ("deep" * 24)
            completed, evidence, _pid_file = self.run_runner(
                root,
                "tests.fixtures.runner_cases.RunnerCases.test_fast",
                timeout=5,
            ...
            self.assertIn("task-root-too-long", completed.stderr)
            self.assertFalse(evidence.exists())
            self.assertFalse((root / "task").exists())

    def test_rejects_task_root_on_a_non_fixed_volume(self):
        checker = getattr(windows_test_runner, "_require_fixed_ntfs_task_root", None)
        self.assertIsNotNone(checker, "fixed-NTFS task-root preflight is missing")
        with mock.patch.object(
            windows_test_runner.ctypes.windll.kernel32,
            "GetDriveTypeW",
            return_value=2,
        ):
            with self.assertRaisesRegex(SystemExit, "task-root-volume-not-fixed-ntfs"):
                checker(Path("C:/AOQ/task"))
