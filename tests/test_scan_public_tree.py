...
from scripts.scan_public_tree import main, scan_content, scan_tree
...
class ScanPublicTreeTests(unittest.TestCase):
    ...
    def test_scan_content_preserves_python_labels_and_binary_exemption(self):
        private_python = text("owner", " = 'public'\n").encode()
        self.assertEqual(
            scan_content("settings.py", private_python),
            [scanner.Finding("settings.py", "content", "private")],
        )
        portable_executable = bytearray(128)
        portable_executable[:2] = b"MZ"
        portable_executable[0x3C:0x40] = (0x40).to_bytes(4, "little")
        portable_executable[0x40:0x44] = b"PE\0\0"
        self.assertEqual(scan_content("runtime.dll", portable_executable), [])
        self.assertEqual(
            scan_content("runtime.dll", text("token", "=plaintext").encode()),
            [scanner.Finding("runtime.dll", "content", "private")],
        )
        self.assertEqual(scan_content("invalid.txt", b"public\xff\xfe"), [])
    ...
    def test_propagates_walk_and_read_errors(self):
        ...
        self.write("read.txt")
        with patch("scripts.scan_public_tree.Path.read_bytes", side_effect=OSError("read")):
            with self.assertRaises(OSError): scan_tree(self.root)
