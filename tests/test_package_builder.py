    ...
    def test_public_package_contains_license_and_notice(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with zipfile.ZipFile(archive) as package:
                self.assertTrue({"LICENSE", "NOTICE"} <= set(package.namelist()))

    def test_public_package_requires_license_and_notice(self):
        import scripts.build_preview as builder
        ...
        for missing in ("LICENSE", "NOTICE"):
            with self.subTest(missing=missing), tempfile.TemporaryDirectory() as temporary:
                ...
                with (
                    mock.patch.object(builder, "_LOCK_PATH", lock_path),
                    mock.patch.object(builder, "_S01_LOCKS", identities),
                    self.assertRaisesRegex(ValueError, "missing bootstrap contract"),
                ):
                    builder.build_preview(
                        source,
                        components["ao2"][1],
