...
B_REQUIREMENTS = [(f"B{number:02d}", f"V1.2 blocker B{number:02d} closure.") for number in range(1, 20)]
...
class VerifyRequirementsTests(unittest.TestCase):
    ...
    def rows(self):
        return [
            {"id": requirement_id, "description": description, "test_id": f"tests.test_{requirement_id.lower()}", "release_phase": "month-1"}
            for requirement_id, description in V11_REQUIREMENTS + B_REQUIREMENTS
        ]

    def write_requirements(self, rows=None):
        self.path.write_text(json.dumps({"schema_version": 1, "requirements": self.rows() if rows is None else rows}))
        return self.path
    ...
    def test_rejects_whitespace_only_descriptions_and_test_ids(self):
        for field in ("description", "test_id"):
