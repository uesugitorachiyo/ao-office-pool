...
function Assert-ArchiveManifest {
    ...
    try { $entry = $zip.GetEntry($ManifestName); if ($null -eq $entry) { throw 'archive has no preview manifest' }; $reader = [System.IO.StreamReader]::new($entry.Open()); try { $raw = $reader.ReadToEnd() } finally { $reader.Dispose() } } finally { $zip.Dispose(); $stream.Dispose() }
...
function Restore-PreviousInstall {
    param([string]$Root, [string]$Quarantine)
    if (-not (Test-Path -LiteralPath $Root) -and (Test-Path -LiteralPath $Quarantine)) {
...
function Invoke-AtomicUninstall {
    ...
    $quarantine = "$safeRoot.uninstalled.$([guid]::NewGuid().ToString('N'))"
    try {
        Move-Item -LiteralPath $safeRoot -Destination $quarantine
        Assert-InstalledTree $quarantine $archiveManifest.manifest $archiveManifest.manifest_sha256
        Assert-AllFree $quarantine $archiveManifest.manifest.runtime_version
        Assert-MutableStateShape $quarantine
    }
    catch {
        Restore-PreviousInstall $safeRoot $quarantine
        throw
    }
    [pscustomobject]@{
        action = 'Uninstall'
        ...
        preserved_tree = $quarantine
    }
}

Invoke-AtomicUninstall $InstallRoot
